...
#include "minecraftpe/world/level/gen/ChunkViewSource.h"
...
TwilightRandomLevelSource::TwilightRandomLevelSource(Level* level, Dimension* dimension, unsigned int seed)
...
float* TwilightRandomLevelSource::getHeights(float* densities, int x, int y, int z, int sizeX, int sizeY, int sizeZ)
{
...
void TwilightRandomLevelSource::prepareHeights(int x, int z, LevelChunk& chunk)
{
	int i = 2;
	int j = i + 1;
	int k = 33;
...
void TwilightRandomLevelSource::buildSurfaces(LevelChunk& chunk, ChunkPos chunkPos)
{
	uint8_t* rawBlocks = chunk.getRawBlocks();
	for(int k = 0; k < 16; k++)
	{
...
void TwilightRandomLevelSource::loadChunk(LevelChunk& chunk)
{
